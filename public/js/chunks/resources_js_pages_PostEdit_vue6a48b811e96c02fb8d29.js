  ...
  methods: {
    ...
    update: function update() {
      ...
      window.axios.put('/post/' + this.post.id, this.post).then(function (r) {
        ...
        var newPhotos = _this3.$ref.loader.getPhotos();

        if (newPhotos.length) {
          var formData = new FormData();
          newPhotos.forEach(function (photo, i) {
            var response = fetch(newPhoto.url);
            var blob = response.blob();
            formData.append('apartment_photos[' + i + ']', blob, photo.name);
          });

          try {
            var _window$axios$post = window.axios.post('/post/' + _this3.post.id, formData),
                data = _window$axios$post.data;
          } catch (e) {
            console.warn(e);
