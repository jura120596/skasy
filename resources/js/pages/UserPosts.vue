<template>
    <v-container class="cover">
        <v-toolbar-title align="center" justify="center" class="mb-2"
                         v-text="$store.state.auth.user.role === 1024 ? 'Обращения пользователей' : 'Ваши обращения'">
        </v-toolbar-title>
            ...
            <v-layout class="d-flex flex-row flex-wrap">
                ...
                <v-flex
                    ...
                    <v-card
                            ...
                            class="ma-1"
                    >
                        <div v-if="$store.state.auth.user.id === post.id"
...
<script>
    export default {
        ...
        methods: {
            getPage() {
                window.axios.get('/user/post/', {params: {page: this.page, per_page: 10}}).then((response) => {
